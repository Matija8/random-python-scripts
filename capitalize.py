...
def capitalize_files(files: List[str]=[], sort=False, overwrite=False,
        quiet=True, remove_dup=True, stream: TextIO=sys.stderr) -> None:
    '''
        ...
        try:
            with open(file, 'r') as lines:
                ...
                if overwrite:
                    ...
                    write_to_file_overwrite(make_out_name(file, sorted=sort), lines)

                log(f'File: {file} done!')

        except FileNotFoundError:
            log_important('Input file: {file} not found. Please confirm file name exists.')
...
def capitalize_words_brute(line: str) -> str:
    '''
    Capitilize first letter of each word in line.
    Doesn't truncate whitespace. Old code.
    '''
